...
class StereoCamera(SingletonConfigurable):
    ...
    def capture_left(self):
        while not rospy.is_shutdown():
            ...
            if left_re:
                ...
                self.left_image_pub.publish(self.frame_to_ros_image(self.left_image_raw, "left"))
            else:
    ...
    def capture_right(self):
        while not rospy.is_shutdown():
            ...
            if right_re:
                ...
                self.right_image_pub.publish(self.frame_to_ros_image(self.right_image_raw, "right"))
            else:
                rospy.logwarn("Failed to capture left frames.")
    ...
    def generate_point_cloud(self):
        ...
        while not rospy.is_shutdown():
            ...
            if not self.left_image_raw.data and not self.right_image_raw.data:
                rospy.loginfo("No data found on both images, retrying.")
                continue
            elif not self.left_image_raw.data:
                rospy.loginfo("No data found on left image, retrying.")
                continue
            elif not self.right_image_raw.data:
                rospy.loginfo("No data found on right image, retrying.")
                continue
            else:
                left_image = self.ros_to_opencv(self.left_image_raw)
                ...
                Q = np.array([- [1.0, 0.0, 0.0, -6215.689113616943]
                            [0.0, 1.0, 0.0, -1295.1428604125977]
                            [0.0, 0.0, 0.0, 960.5951940692499]
                            [0.0, 0.0, 3.7434033774582964, -0.0]])
    ...
    def ros_to_opencv(ros_image, encoding="bgr8"):
        # Get the image dimensions and encoding
        width = ros_image.width
        height = ros_image.height
        
        # Convert the raw data into a NumPy array
        img_data = np.frombuffer(ros_image.data, dtype=np.uint8)
        
        # Reshape the array to the image dimensions (height, width, channels)
        if encoding == "bgr8":
